...
public interface SingerRepository extends JpaRepository<SingerEntity, Long> {
	...
	@Query("SELECT new com.morlandalex.jdbc_hibernate_jpa_5.data.SingerSummary(s.firstName, s.lastName, a.title) FROM SingerEntity s "
			+ "LEFT JOIN s.albumEntities a WHERE a.releaseDate=(SELECT max(a2.releaseDate) FROM AlbumEntity a2 WHERE a2.singerEntity.id=s.id) ORDER BY s.id")
	List<SingerSummary> findAllSingerSummary();
	
	@Query("SELECT new com.morlandalex.jdbc_hibernate_jpa_5.data.SingerSummary(s.firstName, s.lastName, a.title) FROM SingerEntity s "
			+ "LEFT JOIN s.albumEntities a WHERE a.releaseDate=(SELECT max(a2.releaseDate) FROM AlbumEntity a2 WHERE a2.singerEntity.id=s.id) AND s.id=:id")
